...
- **accounts** — `id`, `provider_id`, `name`, `type` (`loan`/`interest_account`/`bank_account`/`asset_account`), `balance`, `interest_rate`, `coin`, `finish_date` (TEXT, `YYYYMMDD`, loan accounts only)
...
**Input validation and error handling:** the worker trims string inputs, validates resource names, enum values, numeric fields, dates, roles, usernames, and password lengths at the API boundary. Usernames accept only `3–50` characters from `[a-zA-Z0-9_.-]`; passwords must be `8–50` characters; asset/provider/account/goal names are length-limited. Invalid requests return a controlled `{ error: "..." }` response, and unexpected server errors are logged server-side while the client receives only `Internal server error.` rather than database or provider details.
...
| POST | `/api/snapshots/clean-months` | user | Delete older snapshots while keeping the most recent snapshot per month, excluding the current month. |
| POST | `/api/snapshots/clean-years` | user | Delete older snapshots while keeping the most recent snapshot per year, excluding the current year. |
...
- `providerCount` / `accountCount` — stored in snapshot payloads for metadata reference.
- `globalValue` — `totalPortfolioValue()` (the `#portfolioValue` card).
- `topGoalValue` / `topGoalSubtext` — **Top Goal Status** card: shows completion percentage of the top-ranked active goal (`order_by`), dynamically updated in live mode and snapshot views (using `goalProgressFromSnapshot(topGoal, d.accounts)`).
- `allTimeGrowthValue` / `allTimeGrowthSubtext` — **Growth** card: clickable card (`#growthCard`) that cycles between **All-Time Growth**, **YTD Growth**, and **Month Growth**:
  - **All-Time Growth**: baseline is the earliest recorded snapshot, displays net change and average monthly growth pace (`+€X.XX / month`).
  - **YTD Growth**: baseline is the first (oldest) snapshot of the current or selected year, displays net change and average monthly growth pace (`+€X.XX / month`).
  - **Month Growth**: baseline is the first (oldest) snapshot of the current or selected month, displays net change and daily growth pace (`+€X.XX / day`).
...
- **History modal (line chart):** a **📈 History** button (`#historyBtn`) in the Time Travel modal opens the history modal (`#historyModalOverlay`). It has two combo boxes — **Chart type** (`#historyChartType`: Global / By Type / By Provider / By Account) and **Zoom** (`#historyZoom`: All / Monthly / Yearly) — and a Chart.js **line chart** (`#historyChart`). On open, it renders from the in-memory/local snapshot list (`timeTravelList` → `historyData`) without another API read. The list is refreshed only during startup or explicit manual refresh, and updated after snapshot mutations. Closing the modal destroys only the chart; the loaded snapshot data is retained. A **maximize** button (`#maximizeHistoryBtn`, ⛶) to the left of the close button toggles the modal to fullscreen (`toggleHistoryMaximize()` adds a `maximized` class to the overlay; the chart resizes to fill the space).
  - **Chart type:** **Global** plots three lines — `globalValue`, `debit`, `credit`. **By Type** plots one line per asset type from each snapshot's `byType`. **By Provider** plots one line per provider from each snapshot's `byProvider`. **By Account** plots one line per account from each snapshot's `accounts` array, with the top 9 accounts shown individually and the remainder combined into **Others**.
...
- **Account history modal (per-account chart):** on the live Dashboard, each **Account Overview** card (`renderDashboardAccounts()`) becomes **clickable** (`.account-card.clickable`, with a `data-account-history` attribute and a "View account history" tooltip) **only when the user has at least one snapshot** (`timeTravelList.length > 0`). Clicking a card opens the account history modal (`#accountHistoryModalOverlay`), which mirrors the Snapshot History modal's design: a **maximize** button (`#maximizeAccountHistoryBtn`, ⛶) and a **close** button (`#closeAccountHistoryBtn`, ✕) in the header, a **Zoom** select (`#accountHistoryZoom`: All / Monthly / Yearly), a loading spinner (`#accountHistoryLoading`), a Chart.js **line chart** (`#accountHistoryChart`), and an empty state (`#accountHistoryEmpty`). The title shows the account name (e.g. "📈 Cash — History"). On open, `openAccountHistoryModal(accountId)` uses the stored snapshot list without an API read, then `renderAccountHistoryChart()` plots the selected account's `valueEur` from each snapshot's `accounts` array over time (chronological x-axis, `spanGaps: true` for snapshots where the account wasn't present). `closeAccountHistoryModal()` destroys only the chart and retains the loaded data. The maximize/restore behavior (`toggleAccountHistoryMaximize()`) and CSS mirror the history modal.
- **Account Overview border color by change:** on the main dashboard (live or in Time Travel), each **Account Overview** card's border is colored by its value change vs the previous snapshot: **green** (`.account-card.account-up`, `--accent2`) when the account's EUR value **increased**, **red** (`.account-card.account-down`, `--danger`) when it **decreased**. `previousAccountValue(prevData, accountId)` looks up the account's `valueEur` in the previous snapshot's `accounts` array (by id); `accountChangeClass(current, prev)` returns `account-up` / `account-down` / `''`. The previous snapshot is the same one used for the Global Value delta (`getPreviousSnapshotData()`). No border color is applied when the value is **unchanged**, when the account **wasn't present** in the previous snapshot, on the **oldest** snapshot, or when there are **no snapshots** (e.g. guest mode).
...
- **Dashboard growth selector:** the clickable `#growthCard` cycles through `All-Time Growth`, `YTD Growth`, and `Month Growth`. The selected mode is stored in `localStorage` as `portfolio_growth_card_mode`, so it survives reloads. All-Time uses the oldest available snapshot; YTD uses the oldest snapshot in the selected/current year; Month uses the oldest snapshot in the selected/current month and falls back to that year's oldest snapshot when the month has no snapshot. All-Time and YTD show an average monthly pace, while Month shows an average daily pace.
- **Dashboard breakdown selector:** the clickable `#dashboardBreakdownCard` cycles the second doughnut chart between **By Provider** and **By Account**. The selected mode is stored in `localStorage` as `portfolio_dashboard_breakdown_mode`, and both modes retain the top 9 plus **Others** grouping and clickable filtering.
- **Collapsible Account Overview:** the Dashboard **Account Overview** is expanded by default and can be toggled with `#toggleDashboardAccountsBtn`. Its summary shows the displayed account count plus up/down/unchanged status compared with the previous snapshot, followed by the top three movers ranked by absolute EUR change. Accounts without a comparison are counted as unchanged and are excluded from the movers list.
...
- **Data loading:** `loadData({ refreshSnapshots })` fetches the portfolio collections in parallel via `Promise.all`. Startup/login and the explicit refresh button pass `refreshSnapshots: true`, which also awaits a complete snapshot refresh. Normal CRUD reloads use the default false, preserving the stored snapshot list and avoiding an unnecessary snapshot API read.
...
**My Accounts** is a merged page combining providers and their accounts. It lists providers as **provider cards** (`.provider-card`), each with a styled header (`.provider-card-head`) showing a collapse toggle, provider icon, name, type tag, account count, the provider's **total value in EUR** (`.provider-total`, via `providerValue`), and icon-only action buttons (`.action-icon-btn`): **➕ Add Account** (green `.add`, leftmost), **ℹ️ Details**, **✏️ Edit**, **🗑️ Delete**. The body (`.provider-card-body`) contains the accounts nested inside so they visually belong to the provider. Each **account card** also uses icon-only buttons: **ℹ️ Details** (asset accounts with holdings only), **⚡ Loan simulator** (loan accounts only), **✏️ Edit**, **🗑️ Delete**.

- **Account value display:** every account card shows its value on the **left** of the detail grid (asset accounts show **Value** then **Holdings**; loan/interest accounts show **Balance** then **Interest Rate**; bank accounts show **Balance**). **Loan** accounts additionally show a **Finish Date** (the account `finish_date` formatted as `YYYY-MM-DD`, or `—` when missing). When an account's currency is not EUR, the converted **EUR value is shown in parentheses** after the value (e.g. `$1,234.56 (€1,050.00)`).
...
- **Account modal fields:** the account create/edit modal (`#accountForm`) shows a **Finish Date** date input (`#accountFinishDateInput`, `name="finish_date"`) **only for `loan` accounts** (shown/hidden by `toggleAccountFields` alongside the balance and interest-rate fields). The date input uses `YYYY-MM-DD`; on save it is converted to the stored `YYYYMMDD` format via `finishDateToDb` (and back via `finishDateToInput` when editing). The API validates it as an 8-digit string and stores it only for loan accounts (null otherwise).
...
- **Loan simulator:** every **loan** account card shows a **⚡** button (`data-loan-sim`, `title="Loan simulator"`). The button is **disabled** (`.btn-sm:disabled`) when the account is missing any of the values the simulation needs — `balance`, `coin`, `interest_rate`, or `finish_date` — and `openLoanSimModal` also guards against opening in that state. Clicking it opens `openLoanSimModal(accountId)` — a modal (`#loanSimModalOverlay`) that lets the user model an **early amortization** of the loan. The modal header shows the account name (`#loanSimTitle`), and a summary (`#loanSimSummary`, styled with `.goal-sim-summary`) shows the **Remaining credit** (the account `balance` absolute value, in EUR), the **TAEG** (the account `interest_rate`, defaulting to `0.00%` when null), and the **Finish date** (the account `finish_date` formatted as `YYYY-MM-DD`). The **end date is taken automatically from the account's `finish_date`** (converted via `finishDateToInput`), so the user provides only the **Amount to amortize** (`#loanSimAmount`, in EUR). On **Calculate** (`#loanSimForm` submit → `runLoanSimulation`), the modal renders **three scenarios** side by side in a `.loan-sim-grid` (three `.loan-sim-section` cards, the last one highlighted with `.loan-sim-highlight`):
  - **Current scenario** — payments remaining (whole months between today and the end date, via `loanMonthsUntilDate`), interest in the next payment, principal amortized, monthly payment, and total payments to end.
  - **Amortize & keep the term** — the new (lower) monthly payment after applying the amortization to the capital, the monthly reduction, total interest saved, total payments to end, and total including the amortization. Interest and principal breakdowns include their percentage of the monthly payment.
  - **Amortize & keep the payment** — the new estimated payoff month/year and months saved, the new interest/principal breakdown with percentages, total interest saved, total payments to end, and total including the amortization.
  The math is ported from the reference amortization calculator: `loanPayment(capital, annualRate, months)` computes the fixed monthly payment (`capital·r/(1−(1+r)^−n)`, or `capital/months` when the rate is 0) and its interest/principal split; `loanRemainingTerm(capital, monthlyPayment, annualRate)` inverts it (`−log(1−r·capital/pmt)/log(1+r)`, or `capital/pmt` when the rate is 0). Validation rejects a non-future end date, a negative amortization, or an amortization exceeding the remaining credit, showing "Check the values and choose a future date for the last payment." in the form's `.form-error`. All monetary values are formatted in **EUR** via `formatCurrency`.

  The result also includes a month-by-month evolution table for the keep-the-term scenario. Summary chips show the estimated payoff date, total interest paid, and total interest saved; the table shows the recalculated payment, interest, principal, and remaining balance through payoff. Interest/principal details use the muted `.detalhe-prestacao` style, the table scrolls horizontally on narrow screens, and `.loan-sim-inputs-grid` responsive breakpoints adapt the input layout. The modal closes via the header **✕** (`#closeLoanSimX`).
...
- **Mechanism:** `applyBlur()` toggles a `blur-mode` class on `<body>` and the `.active` class on the privacy button, then calls `blurNumbers()`/`unblurNumbers()`. `blurNumbers()` walks text nodes, stores the original amount in `span.dataset.orig`, and replaces the text content with a uniform 3-digit placeholder (`888`) wrapped in `<span class="blur-num">` (CSS `filter: blur(5px)`), leaving the currency symbol readable and hiding the order of magnitude. `unblurNumbers()` restores `span.dataset.orig` as a text node and calls `root.normalize()` to merge adjacent text nodes back into a single node — this is essential so the currency symbol and amount are adjacent again and re-blurring works. A `MutationObserver` (`initBlurObserver`) re-applies the blur automatically after any re-render (e.g. `loadData()`/refresh), so it stays correct across page changes and data updates.
- **Currency-only detection:** the regex `BLUR_CURRENCY_RE = /([€$£¥₹₽₩₺₴₦฿₫₪₱₲₡₵₸₼₾₿¤])\s*(\d[\d.,]*)/g` matches a currency symbol followed by a number, and only the number is blurred into `888`. So `€23,733.29 · 24.4%` blurs to `€888 · 24.4%` (blurred so the digits look identical in size, while `€`, signs, and `24.4%` stay readable), while quantities (`10`), percentages (`+5.66%`), and other bare numbers are left untouched. Unblurring restores the exact original value `€23,733.29`.
