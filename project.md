...
The Loan Simulator opens for loan accounts that have a balance, currency, interest rate, and finish date. It takes the outstanding credit, annual interest rate, last payment date, an optional downpayment/amortization, an optional monthly extra amortization, and an optional final payment threshold. Rates are annual and monthly payments are constant. The inputs are laid out three per row: outstanding credit, annual interest rate, and last payment date on the first row; downpayment/amortization, monthly extra amortization, and final payment threshold on the second.

It renders three comparison cards — Current scenario, Amortize & keep the term, and Amortize & keep the payment — followed by a month-by-month evolution table for the "keep the term" scenario. In that scenario the base payment is recalculated each month from that month's balance and the remaining months to the original end date, so it decreases month by month whenever there is extra monthly amortization.

**Amortization timing.** The downpayment and the monthly extra amortization are both applied *after* that month's regular payment, never before it or at the same time. A 5000 downpayment on a 5000 loan therefore does not settle it on day one: month 1 charges interest on the full 5000 and takes the normal payment, and only then is the downpayment deducted from what is left. The comparison cards follow the same rule, so the "keep the term" and "keep the payment" figures are computed from the balance remaining after the first regular payment and the downpayment.

When both extras are used, the monthly extra waits: it is skipped in month 1 and starts in month 2, so the downpayment gets the first month to itself. Used on its own, the monthly extra starts in month 1 like the downpayment does. On a 5000 loan at 4% over 166 months, a 1000 downpayment plus a 100 monthly extra gives month 1 an amortization of 1022.60 (22.60 principal + 1000 downpayment, no monthly extra) and month 2 an amortization of 118.12 (18.12 principal + 100 monthly extra).

**Final payment threshold.** When the optional threshold is set, `gerarEvolucaoComAmortizacaoMensal()` settles the loan in full once the balance after a normal payment lands at or below it. The settling happens the month *after* the balance reaches the threshold (Option A): that month charges interest on the remaining balance plus the whole balance as principal, leaving nothing outstanding. Both the trigger row and the settlement row carry `pagamentoFinal: true`; the settlement row is the last row and is the one displayed. It renders with the `loan-sim-final-row` class and shows a `loan-sim-final-tag` pill reading "Last payment" in place of the remaining balance.

A **Last Payment** summary chip appears in the chip row only when the threshold actually triggers. It shows the settlement amount and date, plus months and interest saved, computed by re-running the same simulation with the threshold disabled and diffing the two runs. A threshold that is blank or zero never triggers and leaves the output unchanged.

The modal resets every input — including the final payment threshold — each time it opens.
