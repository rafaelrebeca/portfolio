...
- `allTimeGrowthValue` / `allTimeGrowthSubtext` — **All-Time Growth** card: shows monetary gain/loss and percentage net worth change compared to the earliest recorded snapshot (`timeTravelList[timeTravelList.length - 1]`), along with average monthly growth pace (`+€X.XX / month`) computed as total growth divided by elapsed months since baseline.
