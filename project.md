...
- Pagination: `.snapshot-pagination` (shared by the Currency table and the Time Travel snapshot list) centers the controls; the Currency page collapses far-away page numbers into an ellipsis (`.currency-page-ellipsis`, muted `…`) so the **← / → arrows stay visible** on narrow screens.
