...
- **Loan simulator:** every **loan** account card shows a **⚡** button (`data-loan-sim`, `title="Loan simulator"`). Clicking it opens `openLoanSimModal(accountId)` — a modal (`#loanSimModalOverlay`) that lets the user model an **early amortization** of the loan. The modal header shows the account name (`#loanSimTitle`), and a summary (`#loanSimSummary`, styled with `.goal-sim-summary`) shows the **Remaining credit** (the account `balance` absolute value, in EUR) and the **TAEG** (the account `interest_rate`, defaulting to `0.00%` when null). The user provides only the **Date of last payment** (`#loanSimEndDate`, a future date) and the **Amount to amortize** (`#loanSimAmount`, in EUR). On **Calculate** (`#loanSimForm` submit → `runLoanSimulation`), the modal renders **three scenarios** side by side in a `.loan-sim-grid` (three `.loan-sim-section` cards, the last one highlighted with `.loan-sim-highlight`):
