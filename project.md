...
- **Data loading:** `loadData()` fetches the portfolio collections in parallel via `Promise.all`, calls `render()` immediately, and loads the authenticated user's snapshot list asynchronously so the main app is usable while Time Travel data is fetched.
