...
The toolbar provides previous/next month buttons, a month/year label that opens a picker, a **Today** button, and an asset-accounts toggle. The picker shows a year stepper and a twelve-month grid; selecting a month closes the picker and renders that month. The month summary chips show the net change for the month, the count of up and down days, and the best and worst single day. The best/worst chips are hidden on narrow screens.

The asset-accounts toggle switches daily growth between **Assets included** (every account, the default) and **Assets excluded** (asset accounts omitted, so only cash, deposits, and loans count). The active mode is highlighted and stored in `localStorage` as `portfolio_growth_calendar_asset_mode`, so it survives reloads. Both variants are precomputed by the shared growth cache, so switching is instant and does not recalculate. The toggle affects the Calendar only; the History chart's **By Growth** series always uses the all-accounts variant.
